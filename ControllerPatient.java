...
public class ControllerPatient {
    ...
    private boolean isPrimaryCareSpecialist(String primaryName) throws SQLException {
    	try (Connection con = getConnection();
			PreparedStatement ps = con.prepareStatement("SELECT specialty FROM Doctor WHERE last_name = ?")) {
    ...
    private boolean isPrimaryPediatrician(String primaryName, int patientAge) throws SQLException {
        try (Connection con = getConnection();
        		PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) AS count, specialty FROM Doctor WHERE last_name = ? AND specialty IN ('Pediatrics')")){
        	ps.setString(1, primaryName);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                ...
                if (count > 0 ) {
	                ...
	                if (patientAge > 18 & specialization.equals("Pediatrics")) {
	                	return false;
	                }
	                return true;
                }
